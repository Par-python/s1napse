...
- **OpenStreetMap** — Real-world racetrack outlines for circuits not covered by TUMFTM. © OpenStreetMap contributors, licensed under the Open Database License (ODbL). Data is queried via the Overpass API and processed offline by `tools/import_tracks.py` to generate the geometry of selected entries in `s1napse/tracks/*.json`. Source: <https://www.openstreetmap.org/copyright>.
